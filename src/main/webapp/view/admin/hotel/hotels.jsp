				...
				<c:forEach var="item" items="${listHotels}">
					<tr>
						...
						<td class="d-inline-flex">
							<a class="btn btn-secondary" href="<c:url value='/admin/hotel?action=edit&id=${item.id}'/>"><b>Edit</b></a>
							<a class="btn btn-danger" href="<c:url value='/admin/hotel?action=delete&id=${item.id}'/> " style="margin-left: 16px"><b>Delete</b></a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination justify-content-center">
				<li
					...
					<a class="page-link"
					href="<c:url value ='/admin/hotel?page=1'/>">First</a>
				</li>
				<li
					class="${currentPage > 1 ? 'page-item' : 'page-item disabled'}">
					<a class="page-link"
					href="<c:url value ='/admin/hotel?page=${currentPage - 1}'/>">Previous</a>
				</li>
				<c:forEach var="i" begin="1" end="${lastPage}">
					<li class="${currentPage != i ? 'page-item' : 'page-item active'}">
						<a class="page-link" href="<c:url value='/admin/hotel?page=${i}'/>">${i}</a>
					</li>
				</c:forEach>
				<li
					class="${currentPage < lastPage ? 'page-item' : 'page-item disabled'}">
					<a class="page-link"
					href="<c:url value='/admin/hotel?page=${currentPage+1}'/>">Next</a>
				</li>
				<li
					class="${currentPage < lastPage ? 'page-item' : 'page-item disabled'}">
					<a class="page-link"
					href="<c:url value='/admin/hotel?page=${lastPage}'/>">Last</a>
				</li>
			</ul>
		</nav>
	</div>
